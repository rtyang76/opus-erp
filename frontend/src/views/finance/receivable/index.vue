...
import { getReceivables, createReceivable, updateReceivable, deleteReceivable } from '@/api/finance'
import type { FinReceivable, ReceivableQuery } from '@/api/finance'
...
const statusOptions = [
  ...
  { label: '已收完', value: 'PAID' },
]

// 查询参数
const queryParams = ref<ReceivableQuery>({
  pageNum: 1,
  pageSize: 10,
...
// 表格数据
const tableData = ref<FinReceivable[]>([])
const total = ref(0)
const loading = ref(false)

// 弹窗控制
const dialogVisible = ref(false)
const dialogTitle = ref('新增应收单')
const formData = ref<Partial<FinReceivable>>({
  customerId: 0,
  amount: 0,
  currency: 'CNY',
  dueDate: '',
...
const getList = async () => {
  loading.value = true
  try {
    const { data } = await getReceivables(queryParams.value)
    tableData.value = data.records
    total.value = data.total
  } catch (error) {
    console.error('查询失败:', error)
    ElMessage.error('查询应收单列表失败，请重试')
  } finally {
    loading.value = false
  }
}
...
// 编辑
const handleEdit = (row: FinReceivable) => {
  dialogTitle.value = '编辑应收单'
  formData.value = { ...row }
  dialogVisible.value = true
}
...
const handleSubmit = async () => {
  try {
    if (formData.value.id) {
      await updateReceivable(formData.value.id, formData.value)
