...
import { getOrders, createOrder, auditOrder, shipOrder, cancelOrder } from '@/api/sales'
import type { SoOrder, OrderQuery, OrderCreateRequest } from '@/api/sales'
...
const handleAudit = async (id: number) => {
  try {
    await ElMessageBox.confirm('确定要审核该销售订单吗？', '提示', {
...
const handleCancel = async (id: number) => {
  try {
    await ElMessageBox.confirm('确定要取消该销售订单吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
    ...
    <el-table :data="tableData" v-loading="loading" stripe border>
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 'AUDITED'" type="primary" link @click="handleShip(row.id)">
            发货
          </el-button>
