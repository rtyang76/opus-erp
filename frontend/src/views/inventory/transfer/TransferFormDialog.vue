...
import type { TransferCreateRequest } from '@/api/inventory'
...
const emit = defineEmits<{
  ...
  submit: [data: TransferCreateRequest]
}>()

const formRef = ref<FormInstance>()

// 表单数据
const formData = ref<TransferCreateRequest>({
  fromWarehouseId: 0,
  toWarehouseId: 0,
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate((valid) => {
    ...
    // 组装提交数据
    const submitData: TransferCreateRequest = {
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="调出仓库" prop="fromWarehouseId">
            <el-select v-model="formData.fromWarehouseId" placeholder="请选择调出仓库" style="width: 100%">
              <el-option
        ...
        <el-col :span="12">
          <el-form-item label="调入仓库" prop="toWarehouseId">
            <el-select v-model="formData.toWarehouseId" placeholder="请选择调入仓库" style="width: 100%">
              <el-option
                v-for="item in warehouseList"
                :key="item.id"
                :label="item.warehouseName"
